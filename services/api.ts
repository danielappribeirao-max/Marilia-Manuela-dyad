...
### 3. Alterando Textos da Home Page

Agora, vou aplicar as alterações de texto solicitadas, atualizando as configurações padrão no `services/api.ts` e, em seguida, atualizando o estado da clínica no Supabase para refletir essas mudanças.

**Textos Solicitados:**
*   Título da Home (`heroText`): "Bem Vindos ..."
*   Subtítulo da Home (`heroSubtitle`): "Na Marília Manuela - Centro de Estética Avançada temos tratamentos de ponta, agende seu momento de cuidado em um ambiente de luxo e bem-estar."

Vou atualizar o `services/api.ts` com os novos valores padrão.

<dyad-write path="services/api.ts" description="Atualizando os textos padrão da clínica (heroText e heroSubtitle).">
import { supabase } from '../supabase/client';
import { User, Service, ServicePackage, Booking, Sale, OperatingHours, HolidayException, ClinicSettings, Role } from '../types';
import { FREE_CONSULTATION_SERVICE_ID } from '../constants';
